...
public class TransparencyEventRequest {
    ...
    @JsonAlias({"date", "time", "eventTimestamp", "event_time"})
    private Instant timestamp;

    @NotNull(message = "originComponent is required")
    @JsonAlias({"provider", "source", "component", "origin", "module"})
    ...
    @JsonAlias({"type", "event", "action", "event_type"})
    private String eventType;

    @NotNull(message = "severity is required")
    @JsonAlias({"level", "priority"})
    private Severity severity;

    @NotNull(message = "details map is required")
    @JsonAlias({"metadata", "payload", "data", "info"})
    @NoSensitiveData
